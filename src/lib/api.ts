const API_URLS = {
  agoraToken: 'https://functions.poehali.dev/24538bc7-7de3-4dd6-8ac5-88af3aa0c630',
...
export const api = {
  ...
  async getAgoraToken(channelName: string, uid: number): Promise<{ token: string; appId: string }> {
    const response = await fetch(API_URLS.agoraToken, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
